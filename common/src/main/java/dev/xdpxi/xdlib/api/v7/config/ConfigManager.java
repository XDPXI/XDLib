...
public class ConfigManager {
  ...
  /**
   * Loads a config from disk into a new instance of the given class. If no file exists, saves
   * defaults and returns the default instance. Fields missing from the file keep their default
   * values.
   *
   * <p>Supported field types: boolean, int, long, float, double, String, List
   */
  public static <T extends ConfigData> T load(Class<T> clazz) throws IOException {
    ...
    for (Field field : declaredConfigFields(clazz)) {
      ...
      try {
        ...
        log.warn(
            "[XDLib] - Config '{}': could not load field '{}', keeping default",
            modId,
            field.getName());
      }
    }
  ...
  /**
   * Saves the config instance to config/modid.yml atomically. Comments from @Comment are written as
   * # lines above each field.
   */
  ...
  private static void applyField(Object instance, Field field, Object raw)
    ...
    if (type == boolean.class || type == Boolean.class) {
      ...
      field.set(
          instance, raw instanceof Number n ? n.intValue() : Integer.parseInt(String.valueOf(raw)));
    } else if (type == long.class || type == Long.class) {
      field.set(
          instance, raw instanceof Number n ? n.longValue() : Long.parseLong(String.valueOf(raw)));
    } else if (type == float.class || type == Float.class) {
      field.set(
          instance,
          raw instanceof Number n ? n.floatValue() : Float.parseFloat(String.valueOf(raw)));
    } else if (type == double.class || type == Double.class) {
      field.set(
          instance,
          raw instanceof Number n ? n.doubleValue() : Double.parseDouble(String.valueOf(raw)));
  ...
  private static Config requireAnnotation(Class<?> clazz) {
    ...
    if (ann == null)
      throw new IllegalArgumentException(clazz.getName() + " must be annotated with @Config");
    return ann;
  }
  ...
  private static <T> T newInstance(Class<T> clazz) throws IOException {
    try {
      ...
      throw new IOException(
          "Cannot instantiate config class "
              + clazz.getName()
