...
public final class Register {
  ...
  public static void init() {
    ...
    try {
      Item testItem = registerItem(Item::new, new Item.Properties(), "test_item", Constants.MOD_ID);
    } catch (Exception e) {
      log.error("[XDLib] - Failed to register test armor material:", e);
    }

    try {
      ResourceKey<EquipmentAsset> ARMOR_MATERIAL_KEY =
          ResourceKey.create(
  ...
  private static ResourceKey<Block> createBlockKey(String modId, String itemId) {
    return ResourceKey.create(Registries.BLOCK, Identifier.fromNamespaceAndPath(modId, itemId));
  }

  private static ResourceKey<Item> createItemKey(String modId, String itemId) {
