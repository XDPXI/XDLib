...
public class PopupView extends Screen {
  ...
  private void renderChangelogBox(DrawContext context, int x, int y) {
    ...
    if (this.textRenderer != null && this.changelogText != null) {
      context.drawTextWrapped(
          this.textRenderer, this.changelogText, x + 5, y + 5, boxWidth - 10, 0xFFFFFFFF);
    }
  }
  ...
  private MutableText createChangelogText(String title, String description) {
    ...
    text.append(
        Text.literal(description).setStyle(Style.EMPTY.withColor(Formatting.GRAY).withBold(false)));
